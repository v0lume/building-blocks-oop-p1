...
describe('Shape', () => {
    ...
    it('should fail to be created with 2 points', () => {
        expect(() => new TestShape('red', false, [new Point(0, 0), new Point(0, 3)])).toThrow();
    });

    it('should show it\'s color and filled property', () => {
        expect(new TestShape('blue', false, points).toString())
            .toBe('A Shape with color of blue and not filled. Points: (0, 0), (0, 3), (4, 3).');
        expect(new TestShape('mustard', true, points).toString())
            .toBe('A Shape with color of mustard and filled. Points: (0, 0), (0, 3), (4, 3).');
    });

    it('should have default color and filled property', () => {
        expect(new TestShape(undefined, undefined, points).toString()).toBe('A Shape with color of green and filled. Points: (0, 0), (0, 3), (4, 3).');
    });

    it('should calculate the perimeter', () => {
        expect(new TestShape(undefined, undefined, points).getPerimeter()).toBe(12);
    });
});
